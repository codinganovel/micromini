...
func BufMapEvent(k Event, action string) {
	...
	for i := 0; ; i++ {
		...
		if strings.HasPrefix(a, "command:") {
			...
			// Lua actions removed in micromini - silently skip
			continue
...
// PluginCB is a no-op in micromini since plugins are removed
// Always returns true to allow callbacks to proceed
